...
select
    ...
    trim(v3.modelId), 
    trim(v3.category), trim(v3.tag), trim(v3.tagId), trim(v3.score), trim(v3.totalScore), 
    trim(coalesce(v3.percentile, v3.percentile2)) as percentile, 
    trim(v3.scorePercentile), 
    trim(v3.index),
    trim(v3.mdTag),
...
lateral view json_tuple(api_response, 'status', 'statusCode', 'memberId', 'MemberId', 'lastUpdated', 'scoresInfo', 'Ranked Models') v1 as status, statusCode, memberId, memberId2, lastUpdated, scoresInfo, rankedModels
    lateral view explode(json_split(COALESCE(v1.scoresInfo, v1.rankedModels))) v2 as scoreInfo
        lateral view json_tuple(v2.scoreInfo, 'modelId',  'ModelName', 'modelName', 'category', 'tag', 'tagId', 'score', 'totalScore', 'percentile', 'Percentile', 'scorePercentile', 'Rank', 'rank', 'index', 'mdTag', 'occassion', 'Occassion', 'subBusinessUnit', 'businessUnit', 'scoreDate') v3  as modelId, modelName, modelName2, category, tag, tagId, score, totalScore, percentile, percentile2, scorePercentile, rank, rank2, index, mdTag, occasion, occasion2, subBusinessUnit, businessUnit, scoreDate 
--where
--unix_timestamp(run_date, 'yyyy-MM-dd') >= unix_timestamp(from_unixtime(unix_timestamp() -4*60*60*24, 'yyyy-MM-dd'), 'yyyy-MM-dd')
