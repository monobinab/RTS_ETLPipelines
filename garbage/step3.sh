...
hive -hiveconf load_date=${load_date} -hiveconf current_year=${current_year} -hiveconf current_month=${current_month} -hiveconf current_day=${current_day} -hiveconf yesterday=${yesterday} -f ${sql_dir}/${sql_file} 
ret_code=$?
case ${ret_code} in
