...
/appl/rts/reports/ti/hive_liquidity_daily_table_create.sh > /logs/rts/hive_liquidity_daily_table_create_ti.log  2>&1
ret_code=$?
...
fastload</appl/rts/reports/ti/liquidity_daily.fld > /logs/rts/liquidity_daily_fld_ti.log
ret_code=$?
case ${ret_code} in
...
bteq</appl/rts/reports/ti/insert_into_prod_rts_liquidity.sql > /logs/rts/insert_into_prod_rts_liquidity_ti.log
ret_code=$?
case ${ret_code} in
     0) printf "\nRTS Bteq job Success\n" >> /logs/rts/insert_into_prod_rts_liquidity_ti.log
     *) printf "\nRTS Bteq TI job Failed with Return Code=${ret_code}"| mailx -s "Bteq TI job Failed" -a /logs/rts/insert_into_prod_rts_liquidity_ti.log rtsteam$searshc.com
     exit 1 ;;
esac
