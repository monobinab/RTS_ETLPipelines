...
#outfile = '/staging/msaha/storm_1122.txt'
outfile = '/tmp/storm_1122'
  ...
  try:
    ...
    if line is not None:
		...
		if len(subFields) >= 4 and subFields[2].strip().lower() == "source" :
            ...
            elif "oldPercentile" in field:
                subFields = field.split(":")
                oldPercentile = subFields[1].strip()
            elif "newPercentile" in field:
                subFields = field.split(":")
                newPercentile = subFields[1].strip()

    outputline = headDate + "|" + tailDate + "|" + lid + "|" + modelId + "|" + oldScore + "|" + newScore + "|" + str(minExpiry) + "|" + str(maxExpiry) + "|" + source + "|" + oldPercentile + "|" + newPercentile + "\n"
   # print('%s' % (outputline))
    outfile_hand.write(outputline)
  except:
    #do noting   
    continue;
    #raise;
